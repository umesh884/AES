...
def expand_key(master_key):
    key_columns = [int.from_bytes(master_key[i:i+4], 'big') for i in range(0, 16, 4)]
    i = 4
    while len(key_columns) < 44:
        word = key_columns[-1]
        if i % 4 == 0:
            word = sub_word(rot_word(word)) ^ Rcon[i // 4]
        word ^= key_columns[-4]
        key_columns.append(word)
        i += 1
    round_keys = [b''.join(word.to_bytes(4, 'big') for word in key_columns[i:i+4]) for i in range(0, 44, 4)]
    return round_keys
